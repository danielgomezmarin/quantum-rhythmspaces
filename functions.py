...
# input a 2D pointer s and output the hash it belongs to
	...
	# if x or y are equal to 1 or 0, substract or add a small number to avoid boundary problems
	if x == 1: x = 1 - 1e-10
	if y == 1: y = 1 - 1e-10
	if x == 0: x = 1e-10
	if y == 0: y = 1e-1
	...
	while h_id == -1:
		if (h%r)*w < x <= ((h%r)+1)*w and (h//r)*w < y <= ((h//r)+1)*w:
			h_id = h
		#print(h, h_id, x, y, w, r)
		h += 1
	return h_id+1
...
# input the positions of the pointer "s" and the triangle vertices tri[0,1,2]
# report 1 or 0 if s is inside or outside tri
	...
	if abs(a1-(a2+a3+a4)) > 0.00000001 :  #small number (1xe-10)given floating point imprecission
		inside=0
	else:
		inside=1
	return inside
...
""" --- FUNCTIONS ADDED FOR QUANTUM RANDOM WALKS SONIFICATION ---"""
# These functions are used to sonify the quantum random walks in the file generate_midi.py.

def point_resampling(path, time,  time_max): # this function resamples a point in the path at a given time. It is to be used to match the semiquavers grid
	# path is a list of 2D points, time_max is the total duration of the pattern, time is the value to sample
	N = len(path)
	# first compute the 2 indices below and above time
	i = int(np.floor (time / time_max * N))
	j = i + 1
	...
	w = (time - i * time_max / N) / (time_max / N)
	# then compute the new position
	new_pos_x = pos_i[0] * (1 - w) + pos_j[0] * w
	new_pos_y = pos_i[1] * (1 - w) + pos_j[1] * w

	return np.array([new_pos_x, new_pos_y])


def path_resampling(path, num_semis): # this function resamples a path at num_semis points
	time_max = 1.0
	resampled_path = []
	for i in range(num_semis):
		t_i = i * time_max / num_semis
		resampled_path.append(point_resampling(path, t_i, time_max))
	return resampled_path

def resampled_path2pattern(resampled_path, all_pattlists, pos, triangles, hashed_triangles, hash_density): # this function takes a resampled path and returns a pattern
	final_pattern = []
	for i in range(len(resampled_path)):
		output_pattern = position2pattern(resampled_path[i], all_pattlists, pos, triangles, hashed_triangles, hash_density)
		final_pattern.append(output_pattern[i % 16])
	return final_pattern

def pattern2midipattern(final_pattern, file_name): # this function takes a pattern and returns a midi pattern
	# final_pattern is a list of 16 lists, each list is a step in the pattern
	# file_name is the name of the midi file to be created
	# the function uses the GM_dict to map the instruments to midi notes
	mid = MidiFile()
	track = MidiTrack()
	mid.tracks.append(track)
	track.append(Message('program_change', program=0, time=0))

	# custom list to address silences and overlapping instruments
	custom_list = [	]

	for i in range(len(final_pattern)):
		for note in final_pattern[i]:
			if note != 0:
				custom_list.append([note, i, 0])
				custom_list.append([note, i + 1, 1])

	sorted_data = sorted(custom_list, key=lambda x: (x[1], x[2]))

	# create the midi notes
	track.append(Message('note_on', note=GM_dict[sorted_data[0][0]][2], velocity=100, time=int(sorted_data[0][1] * 120))) # first note to set the time

	for i in range(1,len(sorted_data)):
		note = sorted_data[i][0]
		time_diff = sorted_data[i][1] - sorted_data[i - 1][1]
		if sorted_data[i][2] == 0:
			track.append(Message('note_on', note=GM_dict[note][2], velocity=100, time=int(time_diff * 120)))
		else:
			track.append(Message('note_off', note=GM_dict[note][2], velocity=0, time=int(time_diff * 120)))
